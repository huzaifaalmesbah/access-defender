...
class AccessDefender {
    ...
    public function section_info() {
        echo 'Configure the settings for Access Defender below:';
    }
    ...
    public function warning_title_callback() {
        ...
        echo '<p class="description">Enter the custom warning title (e.g., "Access Denied").</p>';
    }
    ...
    public function check_vpn_proxy() {
        ...
        if ($this->is_vpn_or_proxy()) {
            wp_die($this->get_warning_message(), '', array('response' => 403));
        }
    }
    ...
    private function is_vpn_or_proxy() {
        ...
        return isset($data['proxy']) && $data['proxy'] === true || isset($data['hosting']) && $data['hosting'] === true;
    }
    ...
    private function get_warning_message() {
        ...
        $default_message = '<p>' . __('We\'ve detected that you\'re using a VPN or proxy. 
                            For security reasons, access to this website is not allowed through VPNs or proxies. 
                            Please disable your VPN or proxy and try again.', 'access-defender') . '</p>';
        $message = !empty($this->options['warning_message']) ? wp_kses_post($this->options['warning_message']) : $default_message;
